...
def getDefaultAggregatePath():
    ddir = getDefaultDir()
    return f"{ddir}/aggregates.json"

def getOSName():
    if os.name == "posix":
        return f"{os.uname()[0]}-{os.uname()[4]}"

    if os.name == "nt":
        return f"{platform.platform()}-{platform.architecture()[0]}"

    return "unknown"

def defaultHeaders():
    return {"User-Agent" : f"GENI-LIB {VERSION} ({getOSName()})" }

def getDefaultContextPath():
    ddir = getDefaultDir()
    return os.path.normpath(f"{ddir}/context.json")
...
def disableUrllibWarnings():
    try:
        ...
        # This version of requests doesn't have requests.packages.urllib3 in it
        return
