...
class Request(geni.rspec.RSpec):
    ...
    def __init__ (self):
        ...
        self.add_namespace(Namespaces.VTS)
        self.add_namespace(Namespaces.SDN)
    ...
    def write_xml(self, path):
        f = open(path, "w+")
        f.write(self.to_xml_string(pretty_print=True))
        f.close()
...
class DelayInfo:
    ...
    def _write(self, element):
        ...
        if self.time:
            d.attrib["time"] = str(self.time)
        if self.jitter:
            d.attrib["jitter"] = str(self.jitter)
        if self.correlation:
            d.attrib["correlation"] = str(self.correlation)
        if self.distribution:
            d.attrib["distribution"] = self.distribution
        return d
...
class OVSOpenFlowImage(OVSImage):
    def __init__(self, controller, ofver = "1.0", dpid = None):
        super().__init__("bss:ovs-201-of")
        self.dpid = dpid
        self.controller = controller
        self.ofver = ofver

    def _write(self, element):
        i = super()._write(element)
...
class OVSL2STP:
    ...
    @mode.setter
    def mode(self, val):
        if val not in [OVSL2STP.STP, OVSL2STP.RSTP]:
    ...
    def type(self):
        ...
        # This should not be reached but putting it here to make the linter happy
        raise UnknownSTPModeError(self._mode)
    ...
    def _write(self, element):
        ...
        if self._mode == OVSL2STP.STP:
            se.attrib["type"] = "stp"
            for key, val in self._stp_params.items():
                pe = ET.SubElement(se, "{%s}%s" % (Namespaces.VTS, key))
                pe.attrib["value"] = str(val)
        elif self._mode == OVSL2STP.RSTP:
            se.attrib["type"] = "rstp"
            for key, val in self._rstp_params.items():
                pe = ET.SubElement(se, "{%s}%s" % (Namespaces.VTS, key))
                pe.attrib["value"] = str(val)
        elif self._mode == -1:
...
class Datapath(Resource):
    ...
    def connect_cross_sliver(self, other_dp):
...
class Container(Resource):
    ...
    def __init__ (self, image, name):
        ...
        for ename,ext in Container.EXTENSIONS:
            self._wrapext(ename, ext)
    ...
    def add_ip_rout(self, network, gateway):
...
class InternalCircuit(Port):
    ...
    def _write(self, element):
        ...
        if self.delay_info:
            self.delay_info._write(p)
        if self.loss_info:
            self.loss_info._write(p)
        if self.reorder_info:
            self.reorder_info._write(p)
        t = ET.SubElement(p, "{%s}target" % (Namespaces.VTS.name))
        t.attrib["remote-clientid"] = self.target
        return p
...
class ContainerPort(InternalCircuit):
    def __init__(self, target, vlan = None, delay_info = None, loss_info = None):
        super().__init__(target, vlan, delay_info, loss_info)
        self._v4addresses = []
...
class HgMount(Mount):
    ...
    def __init__ (self, name, source, mount_path, branch = "default"):
        super().__init__("hg", name, mount_path)
...
class SecureHgMount(Mount):
    def __init__ (self, getversion_output, name, source, mount_path, branch = "default"):
        super().__init__("hg-secure", name, mount_path)
        self._source = source
...
class DropboxMount(Mount):
    def __init__ (self, name, mount_path):
        super().__init__("dropbox", name, mount_path)
...
# Utilities #
#############
def connect_internal_circuit (dp1, dp2, delay_info = None, loss_info = None):
    dp1v = None
    dp2v = None
