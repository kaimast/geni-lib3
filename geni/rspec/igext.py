...
class XenVM(Node):
    ...
    def __init__ (self, client_id, component_id = None, exclusive = False):
        super().__init__(client_id, "emulab-xen", component_id = component_id, exclusive = exclusive)
        self.cores = None
        self.ram = None
        self.disk = None
        self.xen_ptype = None
...
class DockerContainer(Node):
    ...
    """
    def __init__ (self, client_id, component_id = None, exclusive = False):
        super().__init__(client_id, "emulab-docker", component_id = component_id, exclusive = exclusive)
        self.cores = None
        self.ram = None
...
class AddressPool(Resource):
    ...
    def _write(self, element):
        pl = ET.SubElement(element, "{%s}routable_pool" % (PGNS.EMULAB.name))
...
class RemoteBlockstore(pg.Node):
    def __init__ (self, name, mount = None, ifacename = "if0"):
        ...
        bs.where = "remote"
        self._bs = bs
        self._interface = self.add_interface(ifacename)
...
class Bridge(pg.Node):
    ...
    def __init__ (self, name, if0name = "if0", if1name = "if1"):
        ...
        self.add_namespace(PGNS.DELAY)

        self.iface0 = self.add_interface(if0name)
        self.pipe0    = self.Pipe();
        self.iface1 = self.add_interface(if1name)
