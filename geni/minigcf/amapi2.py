...
def deletesliver (url, root_bundle, cert, key, creds, slice_urn, options = {}):
    req_data = xmlrpclib.dumps((slice_urn, creds, options), methodname="DeleteSliver")
    return _rpcpost(url, req_data, (cert, key), root_bundle)

def sliverstatus (url, root_bundle, cert, key, creds, slice_urn, options = {}):
    req_data = xmlrpclib.dumps((slice_urn, creds, options), methodname="SliverStatus")
    return _rpcpost(url, req_data, (cert, key), root_bundle)

def renewsliver (url, root_bundle, cert, key, creds, slice_urn, date, options = {}):
...
def createsliver (url, root_bundle, cert, key, creds, slice_urn, rspec, users, options = {}):
